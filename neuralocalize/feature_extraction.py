...
from constants import ICA_THRESHOLD_CONST, DTYPE
...
def run_group_ica_separately(cifti_image, BM, threshold=ICA_THRESHOLD_CONST, num_ic=40, N=91282, load_ica_result=False):
...
def check(path, X, MATLAB_matrix=False):
    if MATLAB_matrix:
        Y = scipy.io.loadmat(path)["T"]
    else:
        Y, _ = utils.cifti_utils.load_cifti_brain_data_from_file(
            path
        )
    try:
        dif = X - Y
    ...
    print(path, "all close", np.allclose(dif, np.zeros_like(dif)))
    print(path, "norm", np.linalg.norm(dif))
    print(path, "mean", np.mean(dif))
    print(path, "std", np.std(dif))
    del Y
    del dif
